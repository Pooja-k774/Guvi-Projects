...
def Aggre_user_plot_1(df,year):
    ...
    fig_bar_1=px.bar(aguyg, x= "Brands", y="Transaction_count",title= f"{year} BRANDS AND TRANSACTION COUNT",width=1000,color_discrete_sequence= px.colors.sequential.Redor_r,hover_name="Brands")
...
def Aggre_user_plot_2(df,quarter):
    ...
    fig_bar_1=px.bar(aguyqg, x= "Brands", y="Transaction_count",title= f"{quarter} QUARTER BRANDS AND TRANSACTION COUNT",width=1000,color_discrete_sequence= px.colors.sequential.Magenta,hover_name="Brands")
...
def Aggre_user_plot_3(df, state):
    augyqs=df[df["States"] == state]

    augyqs.reset_index(drop= True, inplace= True)

    augyqs
    fig_line_1=px.line(augyqs, x="Brands", y="Transaction_count", hover_data="percentage",
...
def Map_insur_District(df, state):
    ...
    col1,col2=st.columns(2)
    with col1:
        fig_bar_1=px.bar(tacyg, x= "Transaction_amount", y="Districts", orientation="h",height=600, title=f"{state.upper()} DISTRICT AND IT'S TRANSACTION AMOUNT",color_discrete_sequence=px.colors.sequential.Cividis_r)

        st.plotly_chart(fig_bar_1)
    with col2:
        fig_bar_2=px.bar(tacyg, x= "Transaction_count", y="Districts", orientation="h",height=600,title=f"{state.upper()} DISTRICT AND IT'S TRANSACTION COUNT",color_discrete_sequence=px.colors.sequential.Magenta_r)
...
def map_user_plot_1(df, year):
    ...
    fig_line_1=px.line(muyg, x="States", y= ["RegisteredUsers","AppOpens"],title=f"{year} REGISTEREDUSER, APPOPENS",width=1000, height=800,markers=True,
...
def map_user_plot_2(df, quarter):
    ...
    fig_line_1=px.line(muyqg, x="States", y= ["RegisteredUsers","AppOpens"],title=f"{df["Years"].min()} YEAR {quarter} QUARTER REGISTEREDUSER, APPOPENS",width=1000, height=800,markers=True,
...
def map_user_plot_3(df, states):
    ...
    with col1:
        fig_map_user_bar_1=px.bar(muyqs, x= "RegisteredUsers",y="Districts",orientation= "h",
                                title= f"{states.upper()} REGISTERED USER",height=800,color_discrete_sequence=px.colors.sequential.haline_r)

        st.plotly_chart(fig_map_user_bar_1)
    with col2:
        fig_map_user_bar_2=px.bar(muyqs, x= "AppOpens",y="Districts",orientation= "h",
                                title= f"{states.upper()} APPOPENS ",height=800,color_discrete_sequence=px.colors.sequential.haline_r)
...
def top_insurance_plot_1(df, state):
    ...
    with col2:
        ...
        fig_top_insur_bar_2= px.bar(tiy, x= "Quarter", y= "Transaction_count", hover_data= "Pincodes",
                                title= "TRANSACTION COUNT", height= 650,width= 600, color_discrete_sequence= px.colors.sequential.Aggrnyl_r)
...
def top_user_plot_1(df,year):
    ...
    fig_top_plot_1=px.bar(tuyg, x= "States", y="RegisteredUsers",color="Quarter", width=1000,height=800, color_discrete_sequence=px.colors.sequential.YlGn_r,hover_name="States",title=f"{year} Registered Users")
...
def top_user_plot_2(df,state):
    ...
    fig_top_pot_2=px.bar(tuys,x="Quarter", y= "RegisteredUsers",title="REGISTERED USERS, PINCODES,QUARTER",width=1000,height=800,color="RegisteredUsers",hover_data="Pincodes",color_continuous_scale=px.colors.sequential.Magenta_r)
...
if select == "HOME":
    ...
    with tab2:
        ...
        if method_2 == "Map Insurance":
           col1,col2=st.columns(2)
           with col1:
                years=st.slider("Select The Year_map insurance",map_insurance["Years"].min(),map_insurance["Years"].max(),map_insurance["Years"].min())
           map_insur_tac_Y=Transaction_amount_count_Y(map_insurance, years)

           col1,col2=st.columns(2)

           with col1:
             states=st.selectbox("Please Select The State", map_insur_tac_Y["States"].unique())
        ...
        with col1:
                ...
                years= st.slider("Select The Years_mu",map_user["Years"].min(), map_user["Years"].max(),map_user["Years"].min())
    ...
    with tab3:
       ...
       if method_3 == "Top Insurance":
            ...
            with col1:
                ...
                quarters= st.slider("Select The Quarter_tu",top_insur_tac_Y["Quarter"].min(), top_insur_tac_Y["Quarter"].max(),top_insur_tac_Y["Quarter"].min())
          ...
          with col1:
                ...
                years= st.slider("Select The Year_trxn",top_transaction["Years"].min(), top_transaction["Years"].max(),top_transaction["Years"].min())
          top_tran_tac_Y= Transaction_amount_count_Y(top_transaction, years)

          col1,col2= st.columns(2)
          with col1:
                states= st.selectbox("Select The State_trxn", top_tran_tac_Y["States"].unique())
          ...
          with col1:
                ...
                quarters= st.slider("Select The Quarter_trxn",top_tran_tac_Y["Quarter"].min(), top_tran_tac_Y["Quarter"].max(),top_tran_tac_Y["Quarter"].min())
          ...
          with col1:
                ...
                years= st.slider("Select The Year_t_user",top_user["Years"].min(), top_user["Years"].max(),top_user["Years"].min())
